namespace Naos.Queueing.Domain
{
    ...
    public class EchoQueueEventHandler : QueueEventHandler<EchoQueueEventData>
    {
        ...
        {
            await Task.Run(() => this.logger.LogInformation($"{{LogKey:l}} {request.Item.Data.Text} (id={request.Item.Id}, type={this.GetType().PrettyName()})", LogKeys.Queueing), cancellationToken).AnyContext();
            return true;
        }
    }
}
