namespace Naos.Sample.Customers.Domain
{
        ...
        : IDomainEventHandler<IDomainEvent> // handles all domainevents
    {
        ...
        {
            await Task.Run(() =>
                this.logger.LogJournal(LogKeys.DomainEvent, $"[{notification.Id}] send {notification.GetType().Name.SliceTill("DomainEvent")}", LogEventPropertyKeys.TrackSendDomainEvent));
        }
    }
}