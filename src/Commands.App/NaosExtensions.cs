namespace Microsoft.Extensions.DependencyInjection
{
    ...
    public static class NaosExtensions
    {
        ...
        public static CommandsOptions AddRequestDispatcher(
            this CommandsOptions options,
            Action<RequestCommandOptions> optionsAction = null,
            bool addDefaultRequestCommands = true)
        {
            ...
            {
                options.Context.Services.AddSingleton<RequestCommandRegistration>(sp =>
                    new RequestCommandRegistration<EchoCommand, EchoCommandResponse> { Route = "api/commands/echo", RequestMethod = "post" });
                options.Context.Services.AddSingleton<RequestCommandRegistration>(sp =>
                    new RequestCommandRegistration<EchoCommand, EchoCommandResponse> { Route = "api/commands/echo", RequestMethod = "get" });
                options.Context.Services.AddSingleton<RequestCommandRegistration>(sp =>
                    new RequestCommandRegistration<PingCommand> { Route = "api/commands/ping", RequestMethod = "get" });
            }

            optionsAction?.Invoke(new RequestCommandOptions(options.Context));
            ...
            options.Context.Services
                .Scan(scan => scan // https://andrewlock.net/using-scrutor-to-automatically-register-your-services-with-the-asp-net-core-di-container/
                    .FromExecutingAssembly()
                    .FromApplicationDependencies(a => !a.FullName.StartsWith("Microsoft", StringComparison.OrdinalIgnoreCase) && !a.FullName.StartsWith("System", StringComparison.OrdinalIgnoreCase))
                    .AddClasses(classes => classes.AssignableTo(typeof(IRequestCommandExtension)), true));
